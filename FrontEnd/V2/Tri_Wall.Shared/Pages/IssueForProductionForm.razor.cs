...
public partial class IssueForProductionForm
{
    ...
    async Task OpenDialogAsync(IssueProductionLine issueProductionLine)
    {
        ...
        if (!result.Cancelled && result.Data is Dictionary<string, object> data)
        {
            ...
            {
                // ViewModel.IssueProductionLine.Add(new IssueProductionLine
                // {
                //     ItemCode =  issueProductionLineDialog.ItemCode,
                //     ItemName =  issueProductionLineDialog.ItemName,
                //     Qty =  issueProductionLineDialog.Qty,
                //     UomName =  issueProductionLineDialog.UomName,
                //     WhsCode =  issueProductionLineDialog.WhsCode,
                //     ManageItem =  issueProductionLineDialog.ManageItem,
                //     Batches =  issueProductionLineDialog.Batches,
                //     Serials =  issueProductionLineDialog.Serials,
                // });
                if (issueProductionLineDialog.LineNum == 0)
                {
                    issueProductionLineDialog.LineNum = ViewModel.IssueProductionLine?.MaxBy(x => x.LineNum)?.LineNum + 1 ?? 1;
                    ViewModel.IssueProductionLine?.Add(issueProductionLineDialog);
                }
                else
                {
                    var index = ViewModel.IssueProductionLine.ToList().FindIndex(i => i.LineNum == issueProductionLineDialog.LineNum);
    ...
    async Task OnSaveTransaction(string type = "")
    {
        ...
        foreach (var line in productionOrder)
        {
                         ...
                         x.ItemCode == line.ItemCode).ToList())
            {
                ...
                ViewModel.IssueProduction.Lines.Add(new IssueProductionLine
                {
                    ...
                    WhsCode = vmIssueProductionLine.WarehouseCode,
        ...
        try
        {
            ...
            if (ViewModel.PostResponses.ErrorCode == "")
            {
                ...
                ViewModel.IssueProduction= new IssueProductionHeader();
